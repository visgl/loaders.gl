...
export const isAsyncIterable: (x: any) => boolean = (x) =>
  Boolean(x) && typeof x[Symbol.asyncIterator] === 'function';
...
export const isResponse: (x: any) => boolean = (x) =>
  ...
  Boolean(x && x.arrayBuffer && x.text && x.json);

export const isFile: (x: unknown) => boolean = (x) => typeof File !== 'undefined' && x instanceof File;
export const isBlob: (x: unknown) => boolean = (x) => typeof Blob !== 'undefined' && x instanceof Blob;
