...
test('fetchFile(Blob)#imports', t => {
  t.ok(fetchFile, 'fetchFile defined');
  t.end();
});

test('fetchFile(Blob)#Response.text()', async t => {
  if (!isBrowser) {
    t.comment('Skip fetchFileBrowser in Node.js');
    t.end();
    return;
  }
  ...
  t.equals(headers.get('content-type'), '', 'response headers has content-type');
...
test('fetchFile(Blob)#Response.json()', async t => {
  if (!isBrowser) {
    t.comment('Skip fetchFileBrowser in Node.js');
    t.end();
    return;
...
test('fetchFile(Blob)#Response.arrayBuffer()', async t => {
  if (!isBrowser) {
    t.comment('Skipping fetchFileBrowser in Node.js');
    t.end();
    return;
  }
...
test('fetchFile(Blob)#Response.body', async t => {
  if (!isBrowser) {
    t.comment('Skipping fetchFileBrowser in Node.js');
    t.end();
    return;
