...
export class I3STilesetTraverser extends TilesetTraverser {
  ...
  shouldRefine(tile: Tile3D, frameState: FrameState) {
  ...
  async _loadTile(nodeId: string, tileset: Tileset3D) {
   ...
   * @param tile - the parent Tile3D instance
   * @param {string} extendedId - optional ID to separate copies of a tile for different viewports.
   *                              const extendedId = `${tile.id}-${frameState.viewport.id}`;
   * @return {void}
   */
  _onTileLoad(header, tile: Tile3D, extendedId: string): void {
