...
import {RequestScheduler, path, LoaderWithParser, LoaderOptions, Tile3DSource} from '@loaders.gl/loader-utils';
...
export class Tileset3D {
  ...
  metadataPromise: ReturnType<Tile3DSource['getMetadata']>;
  ...
  constructor(source: Tile3DSource, options?: Tileset3DProps) {
    // PUBLIC MEMBERS
    this.options = {...DEFAULT_PROPS, ...options};
    this.source = source;
    this.metadataPromise = source.getMetadata();
    ...
    // METRICS
    // The total amount of GPU memory in bytes used by the tileset.
    this.stats = new Stats({id: 'tileset'}); // this.url});
   ...
   */
  async setViewports(viewports: Viewport[] | Viewport | null = null): Promise<number> {
      ...
      this.updatePromise = new Promise<number>((resolve) => {
        setTimeout(() => {
          if (this.lastUpdatedVieports) {
            this._doUpdate(this.lastUpdatedVieports);
          }
  ...
  private _doUpdate(viewports: Viewport[] | Viewport): void {
   ...
   * @returns
   */
  private _calculateViewPropsI3S(): void {
