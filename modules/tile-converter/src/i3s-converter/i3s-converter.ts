...
export default class I3SConverter {
  ...
  writingQueue: WritingQueue<WritingQueueItem> = new WritingQueue();
  ...
  async convert(options: {
    ...
    this.writingQueue = new WritingQueue();
    this.writingQueue.startListening();
  ...
  private async _createAndSaveTileset(outputPath: string, tilesetName: string): Promise<void> {
    ...
    await this.nodePages.save(this.layers0Path, this.writingQueue, isCreateSlpk);
    await this.writingQueue.finalize();
  ...
  private async _convertNodesTree(
    ...
    if (this.isContentSupported(sourceRootTile)) {
      ...
      if (this.options.slpk) {
        ...
        this.writingQueue.enqueue({writePromise: writeFile(childPath, JSON.stringify(child))});
      }
    } else {
  ...
  private async _writeNodeIndexDocument(
    ...
    if (this.options.slpk) {
      ...
      this.writingQueue.enqueue({writePromise: writeFile(rootPath, JSON.stringify(root0))});
    }
  }
  ...
  private async _writeShared(
    ...
    if (this.options.slpk) {
      ...
      this.writingQueue.enqueue({writePromise: writeFile(sharedPath, sharedDataStr)});
    }
  }
