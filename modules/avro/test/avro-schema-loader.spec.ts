...
test('AvroSchemaLoader#parse validates standalone schemas', async () => {
  ...
  await expect(
    AvroSchemaLoaderWithParser.parse(
      new TextEncoder().encode(
        JSON.stringify({type: 'record', name: 'Broken', fields: [{name: 'missing-type'}]})
      ).buffer
    )
...
test('AvroSchemaLoader#parse validates defaults against field schemas', async () => {
  ...
  ).rejects.toThrow(/default.*expected null/);
});
...
test('AvroSchemaLoader#parse accepts recursive named schemas and rejects invalid unions', async () => {
  ...
  await expect(
    AvroSchemaLoaderWithParser.parse(
      new TextEncoder().encode(JSON.stringify(recursiveSchema)).buffer
    )
...
test('Avro schema fingerprints use canonical field order and named references', () => {
  ...
  expect(getAvroSchemaFingerprint({...first, name: 'Other'})).not.toBe(
    getAvroSchemaFingerprint(first)
  );
});
