...
test('NodeHash#hash', async () => {
  if (!isBrowser) {
    ...
    expect(
      hash, 'binary data SHA256 hash is correct'
    ).toBe('gsoMi29gqdIBCEdTdRJW8VPFx5PQyFPTF4Lv7TJ4eQw=');

    hash = await cryptoHash.hash(repeatedData, 'base64');
    expect(
      hash, 'repeated data SHA256 hash is correct'
    ).toBe('bSCTuOJei5XsmAnqtmm2Aw/2EvUHldNdAxYb3mjSK9s=');
  }
});
