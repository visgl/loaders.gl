...
test('cli - geometry-attributes#should return the same attributes with faceRange from 0 to triangleCount - 1 and 0 feature id', async t => {
  if (!isBrowser) {
    const oldAttributes = {
...
test('cli - geometry-attributes#should return the same attributes if we have only one triangle and featureIndices with the same indices', async t => {
  if (!isBrowser) {
    const oldAttributes = {
...
test('cli - geometry-attributes#should return reordered attributes', async t => {
  if (!isBrowser) {
    const oldAttributes = {
      // prettier-ignore
