...
test('TextureLoader#load mipmaps manifest', async (t) => {
  ...
  texture.data.forEach((textureLevel, index) => checkImageTextureLevel(t, textureLevel, `level ${index}`))
  t.end()
})
...
test('TextureArrayLoader#template supports index placeholder', async (t) => {
  ...
  texture.data.forEach((layer, index) => checkImageTextureLevel(t, layer[0], `layer ${index} level 0`))
  t.deepEqual(
    requestedUrls,
