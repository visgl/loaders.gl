...
import {PMTilesLoader, FetchSource} from '@loaders.gl/pmtiles';
import {PMTiles, RangeResponse, SharedPromiseCache} from 'pmtiles';

// inefficient method only for testing
class TestFileSource extends FetchSource {
  key: string;
  etag?: string;
  ...
  async getBytes(offset: number, length: number): Promise<RangeResponse> {
    const result = await super.getBytes(offset, length);
    if (this.etag) {
