...
test('NetCDF source discovers raster metadata from a Blob', async () => {
...
test('NetCDF source reads a remote header with progressive ranges and caches it', async () => {
  const response = await fetchFile(NETCDF_FIXTURE);
...
test('NetCDF source reports remote failures and honors cancellation', async () => {
  const source = new NetCDFSource('fixture.nc');
