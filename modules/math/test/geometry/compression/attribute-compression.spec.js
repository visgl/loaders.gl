...
import {AttributeCompression} from '@loaders.gl/math';
...
it('oct encode(0, 0, 1)', () => {
  const result = new Vector2();
  AttributeCompression.octEncode(VECTOR3_UNIT_Z, result);
  expect(result).toEqual(new Vector2(128, 128));
});

it('oct encode(0, 0, -1) to 4 components', () => {
  const result = new Vector4();
  AttributeCompression.octEncodeToVector4(negativeUnitZ, result);
  expect(result).toEqual(new Vector4(255, 255, 255, 255));
});

it('oct encode(0, 0, 1) to 4 components', () => {
  const result = new Vector4();
  AttributeCompression.octEncodeToVector4(VECTOR3_UNIT_Z, result);
...
it('throws oct encode vector undefined', () => {
  ...
  const result = new Vector3();
  expect(() => AttributeCompression.octEncode(vector, result)).toThrow();
});

it('throws oct encode result undefined', () => {
  const result = [0, 0, 0];
  expect(() => AttributeCompression.octEncode(VECTOR3_UNIT_Z, result)).toThrow();
});

it('throws oct encode non unit vector', () => {
  const nonUnitLengthVector = new Vector3(2.0, 0.0, 0.0);
  const result = new Vector2();
  expect(() => AttributeCompression.octEncode(nonUnitLengthVector, result)).toThrow();
});

it('throws oct encode zero length vector', () => {
  const result = new Vector2();
  expect(() => AttributeCompression.octEncode(Vector3.ZERO, result)).toThrow();
});

it('throws oct decode result undefined', () => {
  const result = undefined;
  expect(() => AttributeCompression.octDecode(0, 0, result)).toThrow();
});

it('throws oct decode x out of bounds', () => {
  const result = new Vector3();
  expect(() => AttributeCompression.octDecode(256, 0, result)).toThrow();
});

it('throws oct decode y out of bounds', () => {
  const result = new Vector3();
  expect(() => AttributeCompression.octDecode(0, 256, result)).toThrow();
});

it('throws 4-component oct decode out of bounds', () => {
  const result = new Vector3();
  expect(() =>
    AttributeCompression.octDecodeFromVector4(new Vector4(256, 0, 0, 0), result)
  ).toThrow();

  expect(() =>
    AttributeCompression.octDecodeFromVector4(new Vector4(0, 256, 0, 0), result)
  ).toThrow();

  expect(() =>
    AttributeCompression.octDecodeFromVector4(new Vector4(0, 0, 256, 0), result)
  ).toThrow();

  expect(() =>
    AttributeCompression.octDecodeFromVector4(new Vector4(0, 0, 0, 256), result)
  ).toThrow();
});
...
it('oct encoding', () => {
  ...
  expect(AttributeCompression.octDecode(encoded.x, encoded.y, result)).toEqualEpsilon(
    normal,
    epsilon
  );
...
it('oct encoding high precision', () => {
  ...
  expect(
    AttributeCompression.octDecodeInRange(encoded.x, encoded.y, rangeMax, result)
...
it('oct encoding to 4 components', () => {
  ...
  const result = new Vector3();
  let normal = new Vector3(0.0, 0.0, 1.0);
  AttributeCompression.octEncodeToVector4(normal, encoded);
  expect(AttributeCompression.octDecodeFromVector4(encoded, result)).toEqualEpsilon(
    normal,
    epsilon
  );
...
it('octFloat encoding', () => {
  ...
  let normal = new Vector3(0.0, 0.0, 1.0);
  expect(
    AttributeCompression.octDecodeFloat(AttributeCompression.octEncodeFloat(normal), result)
  ).toEqualEpsilon(normal, epsilon);
...
it('encodeFloat throws without vector', () => {
  expect(() => AttributeCompression.octEncodeFloat(undefined)).toThrow();
});

it('decodeFloat throws without value', () => {
  expect(() => AttributeCompression.octDecodeFloat(undefined, new Vector3())).toThrow();
});

it('decodeFloat throws without result', () => {
  expect(() => AttributeCompression.octDecodeFloat(0.0, undefined)).toThrow();
});
...
it('encode and packFloat is equivalent to oct encoding', () => {
  ...
  const encoded = AttributeCompression.octEncode(vector, new Vector2());
  const encodedFloat = AttributeCompression.octPackFloat(encoded);
  expect(AttributeCompression.octDecodeFloat(encodedFloat, new Vector3())).toEqual(
    AttributeCompression.octDecode(encoded.x, encoded.y, new Vector3())
  );
});

it('packFloat throws without encoded', () => {
  expect(() => AttributeCompression.octPackFloat(undefined)).toThrow();
});
...
it('pack is equivalent to oct encoding', () => {
  ...
  const packed = AttributeCompression.octPack(x, y, z, new Vector2());
  ...
  expect(decodedX).toEqual(
    AttributeCompression.octDecodeFloat(AttributeCompression.octEncodeFloat(x), new Vector3())
  );
  expect(decodedY).toEqual(
    AttributeCompression.octDecodeFloat(AttributeCompression.octEncodeFloat(y), new Vector3())
  );
  expect(decodedZ).toEqual(
    AttributeCompression.octDecodeFloat(AttributeCompression.octEncodeFloat(z), new Vector3())
  );
});

it('pack throws without v1', () => {
  expect(() =>
    AttributeCompression.octPack(undefined, new Vector3(), new Vector3(), new Vector2())
  ).toThrow();
});

it('pack throws without v2', () => {
  expect(() =>
    AttributeCompression.octPack(new Vector3(), undefined, new Vector3(), new Vector2())
  ).toThrow();
});

it('pack throws without v3', () => {
  expect(() =>
    AttributeCompression.octPack(new Vector3(), new Vector3(), undefined, new Vector2())
  ).toThrow();
});

it('pack throws without result', () => {
  expect(() =>
    AttributeCompression.octPack(new Vector3(), new Vector3(), new Vector3(), undefined)
  ).toThrow();
});

it('unpack throws without packed', () => {
  expect(() =>
    AttributeCompression.octUnpack(undefined, new Vector3(), new Vector3(), new Vector3())
  ).toThrow();
});

it('unpack throws without v1', () => {
  expect(() =>
    AttributeCompression.octUnpack(new Vector2(), undefined, new Vector3(), new Vector3())
  ).toThrow();
});

it('unpack throws without v2', () => {
  expect(() =>
    AttributeCompression.octUnpack(new Vector2(), new Vector3(), undefined, new Vector3())
  ).toThrow();
});

it('unpack throws without v3', () => {
  expect(() =>
    AttributeCompression.octUnpack(new Vector2(), new Vector3(), new Vector3(), undefined)
  ).toThrow();
});
...
it('compress texture coordinates throws without texture coordinates', () => {
  expect(() => AttributeCompression.compressTextureCoordinates(undefined)).toThrow();
});

it('decompress texture coordinates throws without encoded texture coordinates', () => {
  expect(() =>
    AttributeCompression.decompressTextureCoordinates(undefined, new Vector2())
  ).toThrow();
});

it('decompress texture coordinates throws without result', () => {
  expect(() => AttributeCompression.decompressTextureCoordinates(0.0, undefined)).toThrow();
});
...
it('throws when zigZagDeltaDecode has an undefined uBuffer', () => {
  expect(() => AttributeCompression.zigZagDeltaDecode(undefined, new Uint16Array(10))).toThrow();
});

it('throws when zigZagDeltaDecode has an undefined vBuffer', () => {
  expect(() => AttributeCompression.zigZagDeltaDecode(new Uint16Array(10), undefined)).toThrow();
});
