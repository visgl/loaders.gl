...
const scratchEncodeCart2 = new Vector2();
/**
...
export default class AttributeCompression {
   ...
   */
  static octEncodeInRange(vector, rangeMax, result) {
    assert(vector);
    assert(result);
   ...
   */
  static octEncode(vector, result) {
   ...
   *
   * @see AttributeCompression.octEncodeInRange
   * @see AttributeCompression.octDecodeFromVector4
   */
  static octEncodeToVector4(vector, result) {
   ...
   *
   * @see AttributeCompression.octDecodeInRange
   * @see AttributeCompression.octEncodeToVector4
   */
  static octDecodeFromVector4(encoded, result) {
    assert(encoded);
    assert(result);
    const x = encoded.x;
    const y = encoded.y;
  ...
  static octPackFloat(encoded) {
    assert(Number.isFinite(encoded));
   ...
   *
   */
  static octDecodeFloat(value, result) {
   ...
   *
   */
  static octPack(v1, v2, v3, result) {
    assert(Number.isFinite(v1));
    assert(Number.isFinite(v2));
    assert(Number.isFinite(v3));
    assert(result);
   ...
   */
  static octUnpack(packed, v1, v2, v3) {
   ...
   */
  static zigZagDeltaDecode(uBuffer, vBuffer, heightBuffer) {
    assert(Number.isFinite(uBuffer));
    assert(Number.isFinite(vBuffer));
