...
export class FlatGeobufVectorSource extends DataSource<string, FlatGeobufSourceLoaderOptions> implements VectorSource {
  ...
  async getFeatures(parameters: GetFeaturesParameters): Promise<VectorSourceData> {
    ...
    return parseFlatGeobuf(info.arrayBuffer, {shape: format === 'arrow' ? 'arrow-table' : format === 'binary' ? 'binary-geometry' : 'geojson-table', boundingBox: parameters.boundingBox, targetCrs: parameters.crs || 'WGS84', reproject: Boolean(parameters.crs), geoarrow: parameters.geoarrow}) as VectorSourceData;
  }
