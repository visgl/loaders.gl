...
// This code was inspired by https://github.com/mapbox/vector-tile-js under BSD 3-clause license.
...
//   _keys: [],
//   _values: []
// } as const satisfies MVTFeature;

function readGeoJSONFeatureFromPBF(
  vtFeature: VectorTileFeature,
  coordinates: 'wgs84' | 'local',
  extent: number,
  tileIndex?: {x: number; y: number; z: number}
): Feature {
  const coords = vtFeature.loadGeometry();

  switch (coordinates) {
    case 'wgs84':
      return _toGeoJSONFeature(vtFeature, coords, (line: number[][]) =>
        projectToLngLat(line, tileIndex!, extent)
      );

    default:
      return _toGeoJSONFeature(vtFeature, coords, convertToLocalCoordinates);
  }
}
 ...
 * @returns
 */
function readBinaryFeatureFromPBF(
  vtFeature: VectorTileFeature,
  coordinates: 'wgs84' | 'local',
  extent: number,
  tileIndex?: {x: number; y: number; z: number},
): FlatFeature {
  const geom = vtFeature.loadFlatGeometry();

  let geometry: FlatGeometry;
  switch (coordinates) {
    case 'wgs84':
      geometry = readFlatGeometryFromPBF({
        geom,
        transform: (coords: number[]) => projectToLngLatFlat(coords, tileIndex!, extent),
        extent,
        type: vtFeature.type,
        _geometryInfo: vtFeature._geometryInfo
      });
      break;
    ...
    default:
      geometry = readFlatGeometryFromPBF({
        geom,
        transform: convertToLocalCoordinatesFlat,
        extent,
        type: vtFeature.type,
        _geometryInfo: vtFeature._geometryInfo
      });
  ...
  const feature: FlatFeature = {type: 'Feature', geometry, properties: vtFeature.properties};

  if (vtFeature.id !== null) {
    feature.id = vtFeature.id;
  }
 ...
 * @returns result
 */
function readFlatGeometryFromPBF(options: {
  ...
  const {geom, transform, extent, type, _geometryInfo} = options;
  let geometry;

  // Apply the supplied transformation to data
  transform(geom.data, extent);
  ...
  switch (type) {
    case 1: // Point
      _geometryInfo.pointFeaturesCount++;
      _geometryInfo.pointPositionsCount += geom.indices.length;
      geometry = {type: 'Point', ...geom};
      break;

    case 2: // LineString
      _geometryInfo.lineFeaturesCount++;
      _geometryInfo.linePathsCount += geom.indices.length;
      _geometryInfo.linePositionsCount += geom.data.length / coordLength;
    ...
    case 3: // Polygon
      ...
      _geometryInfo.polygonFeaturesCount++;
      _geometryInfo.polygonObjectsCount += geometry.indices.length;

      for (const indices of geometry.indices) {
        _geometryInfo.polygonRingsCount += indices.length;
      }
      _geometryInfo.polygonPositionsCount += geometry.data.length / coordLength;
...
function _toGeoJSONFeature(
  ...
  // TODO more precise typing, handle Unknown type
  let type: string = MVT_GEOMETRY_TYPES[vtFeature.type];
  let i: number;
  let j: number;
...
function readBoundingBoxFromPBF(pbf: Protobuf): [number, number, number, number] {
  const end = pbf.readVarint() + pbf.pos;
  let cmd = 1;
  let length = 0;
  let x = 0;
...
/** */
function loadGeometryFromPBF(pbf): number[][][] {
  const end = pbf.readVarint() + pbf.pos;
  let cmd = 1;
  let length = 0;
  let x = 0;
...
function loadFlatGeometryFromPBF(pbf: Protobuf): FlatIndexedGeometry {
