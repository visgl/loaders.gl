...
function createDeltaBinaryPackedFixture(
  firstValue: bigint,
  minimumDelta: bigint,
  packedDeltas: bigint[],
  bitWidth: number
...
test('ParquetCodec::DELTA_BINARY_PACKED#decodes INT32 values with wrapping semantics', t => {
  ...
  t.equal(cursor.offset, cursor.buffer.length, 'consumes the padded INT32 mini-block');
  t.end();
});

test('ParquetCodec::DELTA_BINARY_PACKED#decodes exact-number INT64 packed values', t => {
  const largestPackedDelta = (1n << 45n) - 1n;
  const buffer = createDeltaBinaryPackedFixture(0n, -1n, [largestPackedDelta, 7n], 45);
  const cursor = {buffer, offset: 0};

  t.deepEqual(PARQUET_CODECS.DELTA_BINARY_PACKED.decodeValues('INT64', cursor, 3, {}), [
    0, Number(largestPackedDelta - 1n), Number(largestPackedDelta + 5n)
  ]);
  t.equal(cursor.offset, cursor.buffer.length, 'consumes the padded number-reservoir mini-block');
  t.end();
});

test('ParquetCodec::DELTA_BINARY_PACKED#decodes wide INT64 packed values', t => {
  ...
  t.equal(cursor.offset, cursor.buffer.length, 'consumes the padded BigInt-reservoir mini-block');
  t.end();
});

test('ParquetCodec::DELTA_BINARY_PACKED#validates physical integer bit widths', t => {
  /** Creates a DELTA_BINARY_PACKED block that declares one invalid mini-block width. */
  const createInvalidWidthFixture = (bitWidth: number): Uint8Array =>
    bytes([0x80, 0x01, 0x04, 0x02, 0x00, 0x00, bitWidth, 0x00, 0x00, 0x00]);

  t.throws(
    () =>
      PARQUET_CODECS.DELTA_BINARY_PACKED.decodeValues(
        'INT32',
        {buffer: createInvalidWidthFixture(33), offset: 0},
        2,
        {}
      ),
    /Invalid DELTA_BINARY_PACKED bit width/
  );
  t.throws(
    () =>
      PARQUET_CODECS.DELTA_BINARY_PACKED.decodeValues(
        'INT64',
        {buffer: createInvalidWidthFixture(65), offset: 0},
        2,
        {}
      ),
    /Invalid DELTA_BINARY_PACKED bit width/
