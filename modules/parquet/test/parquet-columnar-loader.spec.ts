...
test('ParquetColumnarLoader#load alltypes_dictionary file', async (t) => {
  ...
  const data = await load(url, ParquetColumnarLoader, {parquet: {url}, worker: false});

  t.equal(data.length, 2, 'length is correct');
...
test('ParquetColumnarLoader#decimal files', async (t) => {
  ...
  for (const url of urls) {
    const data = await load(url, ParquetColumnarLoader, {parquet: {url}, worker: false});
    t.ok(typeof data.length === 'number');
...
test('ParquetColumnarLoader#load', async (t) => {
  ...
  if (!isBrowser) {
    ...
    for (const {title, path} of SUPPORTED_FILES) {
      ...
      const data = await load(url, ParquetColumnarLoader, {parquet: {url}, worker: true});
