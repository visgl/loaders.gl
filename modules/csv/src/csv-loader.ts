...
async function parseCSV(csvText: string, options?: CSVLoaderOptions) {
  ...
  if (rowFormat === 'object' && !header) {
    ...
    const headerRow = generateHeader(csvOptions.columnPrefix, firstRow.length);
  ...
  const config = {
    ...
    step(results) {
      ...
      if (tableBatchBuilder.isFull()) {
        asyncQueue.enqueue(tableBatchBuilder.getBatch({bytesUsed}) as Batch);
      }
    },
