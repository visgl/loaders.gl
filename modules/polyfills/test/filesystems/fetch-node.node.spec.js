...
test('polyfills#fetchNode() (NODE)', async () => {
  ...
  expect(response.headers, 'fetchNode polyfill successfully returned headers under Node.js').toBeTruthy();
...
test('polyfills#fetchNode() error handling (NODE)', async () => {
  ...
  expect(response.statusText.includes('ENOENT'), 'fetchNode statusText forwards node ENOENT error').toBeTruthy();
  ...
  console.log(response.statusText);
  expect(response.statusText.includes('EISDIR'), 'fetchNode statusText forwards node error').toBeTruthy();
