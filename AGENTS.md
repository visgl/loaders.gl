...
- Coverage from a browser shard or the reduced Node suite is incomplete on its own. Enforce thresholds only after merging all browser shards, and use the merged browser-plus-Node report for Node-only branches. Keep a browser-only ratchet so Node coverage cannot hide a browser regression.
