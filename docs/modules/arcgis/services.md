---
title: ArcGIS service support
description: Operation-level support and gaps across ArcGIS data, location, analysis and enterprise services.
---

# ArcGIS service support

Use this inventory to choose a data path before integrating. `@loaders.gl/arcgis` supplies selected
read operations for visualization, with format and protocol support in other loaders.gl packages.
Each row distinguishes code that exists from missing or unverified integration.
...
| --- | --- | --- | --- | --- |
| [Feature service: layer reads](/docs/modules/arcgis/arcgis-feature-server) | `FeatureServer/{layerId}/query` | Implemented subset | Metadata/schema; one spatial/attribute query; GeoJSON, binary, or Arrow conversion | No automatic paging/completeness contract; limited field mapping; parser requires GeoJSON even though options allow JSON |
...
| Feature service: statistics and advanced queries | Layer `query`, related-record and attachment operations | Not implemented | Basic filtering is available through the feature source | No first-class counts/IDs-only, grouped statistics, related records, attachment browsing, or full temporal-query API |
...
| Map service: feature queries, identify, find and legend | `MapServer/{layerId}/query`, `identify`, `find`, `legend` | Not implemented | Existing MapServer source handles imagery | Cannot treat MapServer vector data as supported just because the service displays as an image |
