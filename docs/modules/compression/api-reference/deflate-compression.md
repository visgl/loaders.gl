...
New code should use `DeflateCompressor` or `DeflateDecompressor`. Select a specific `fflate`,
Pako, or `compress-utils` adapter only when pinning an implementation. Compare them in the
