...
import {ImageTextureLoader} from '@loaders.gl/textures';

const image = await load('texture.image-texture.json', ImageTextureLoader);
```
...
```typescript
import {load} from '@loaders.gl/core';
import {ImageTextureLoader, CompressedTextureLoader, BasisLoader} from '@loaders.gl/textures';
...
Template placeholders are validated strictly. Supported placeholders for `ImageTextureLoader` are `{lod}` only.
...
- `image`: returns one parsed image
- `mipmaps`: returns an array of parsed images, one per mip level
