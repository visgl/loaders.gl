...
`3DObject` and `Point` sublayers are content layers. Those layer types are separated types of layers in I3S spec and can be used independently. Loaders.gl supports both content profiles through the standard I3S source; this example renders the `3DObject` sublayers, while Point metadata and geometry can be inspected with the [I3S profile gallery](./profile-gallery).
