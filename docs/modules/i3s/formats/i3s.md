...
| ArcGIS token propagation | **Supported** | **v5.0** | `core.credentials` applies an exact-origin ArcGIS `token` credential to the root and nested I3S requests, including node pages, geometry, textures, attributes, statistics, and WebScene metadata checks. The legacy `i3s.token` option remains supported; a direct legacy `load(url, I3SLoader, ...)` call still needs the token in the input URL when using only that option because core fetches the root before the parser runs. |
