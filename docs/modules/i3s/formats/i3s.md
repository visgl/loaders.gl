...
| ArcGIS token propagation | **Supported** | **v5.0** | `core.credentials` applies an exact-origin ArcGIS `token` credential to the root and nested I3S requests, including node pages, geometry, textures, attributes, statistics, and WebScene metadata checks. `searchParams: {token}` also supports legacy direct loads by adding the token before the initial request; the legacy `i3s.token` option remains supported for parser and source-managed requests. |
