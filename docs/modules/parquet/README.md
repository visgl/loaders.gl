...
- `DeltaSourceLoader` and `DeltaTableSource` provide read-only Delta snapshot discovery from
  newline-delimited transaction logs, dispatching active Parquet files through the same dataset
  reader.
