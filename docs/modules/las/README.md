...
LAZ point formats 0-10 are supported by the opt-in `typescript` backend, including legacy waveform formats 4/5 and modern LASzip item versions 3/4; support in the `copc` and `laz-rs` backends depends on their bundled decoder versions. The default `laz-perf` backend remains limited to LAS 1.3 and point formats 0-3. See the [LAS/LAZ implementation limits](/docs/modules/las/formats/las#current-implementation-limits) for backend and point-format details.
