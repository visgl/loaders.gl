...
| `options.las.extraBytes` | `'raw'` or `'typed'` | `raw`      | Controls `EXTRA_BYTES` output. `raw` returns the fixed-width byte payload; `typed` returns one `EXTRA_BYTES_<descriptor-name>` numeric Arrow attribute per supported descriptor. 64-bit integer descriptors remain raw-only. |
