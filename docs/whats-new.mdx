...
**Highlights**
- **Tooling** - @vis.gl/dev-tools@1.0, GitHub CI optimizations
- **TypeScript** - Migrate API from `ArrayBuffer` to `ArrayBufferLike`
- **Website** - docusaurus@3.9. Website search. 
- Modernized build tooling
- **TypeScript** Stricter types across code base, migrate to ArrayBufferLike

**All modules**

- Bundled workers now exported explicitly in each module's package.json `exports` section.

**@loaders.gl/core**

- [`LoaderOptions.core`](/docs/modules/core/api-reference/loader-options) - stronger type checks when options are supplied through the new `options.core` sub-option object.
...
- `tile-converter` is now published as an "application", independently of loaders.gl modules.
