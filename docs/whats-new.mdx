...
- **Gaussian splats** - New `@loaders.gl/splats` loaders, Gaussian PLY Arrow table support, and WebGPU `SplatLayer` / `RADSplatLayer` support add experimental rendering for 3D Gaussian splat scenes and Spark RAD LoD sources.
...
- `RADSplatLayer` NEW - renders Spark RAD LoD sources from `RADSourceLoader` through deck.gl/luma.gl without a THREE.js bridge, using paged chunk loading, viewport-driven reselection, a shared GPU render pool for the active frontier, and RAD spherical harmonics when present.
