...
- `WMSCapabilitiesLoader` 
  - now handles version `1.3.0`, and parameter names that have changed across WMS versions are automatically remapped to 1.3.0 equivalents (e.g. `CRS` vs `SRS`).
  - Also extracts more information (such as `version`, `abstract` fields, layer flags (`opaque`, `queryable`, `cascaded`), MIME types for `exceptions`).
- [`WMSService`](modules/wms/api-reference/wms-service) class improvements:
  - WMS `1.3.0` is now the default version.
  - Authentication support: Now correctly uses application supplied `loadOptions`, including fetch overrides and HTTP `Authentication` headers.
