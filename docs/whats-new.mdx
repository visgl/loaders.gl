...
- Async gzip, deflate, raw deflate, Brotli, and Zstandard decompression now probes and uses the runtime's native `DecompressionStream` before falling back to existing codecs. Runtimes with native Zstandard support no longer require `zstd-codec` for async decompression APIs, including Parquet and SPZ parsing.
