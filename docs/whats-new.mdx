...
- [`LASLoader`](/docs/modules/las/api-reference/las-loader) adds opt-in `las.interleaved: true` packed Arrow output for the default laz-perf backend, storing GPU-ready fixed-width vertex records plus buffer layout metadata.

**@loaders.gl/potree**

- `PotreeBinLoader` adds opt-in `potree.interleaved: true` packed Arrow output for binary Potree point tiles, allocating one interleaved vertex buffer per tile and filling it directly during parse.
...
- [`PCDLoader`](/docs/modules/pcd/api-reference/pcd-loader) adds opt-in `pcd.interleaved: true` packed Arrow output that writes directly into preallocated batch buffers for point-cloud rendering.
...
- [`PLYLoader`](/docs/modules/ply/api-reference/ply-loader) adds opt-in `ply.interleaved: true` packed Arrow output for fixed-width binary point-cloud PLY inputs, including custom scalar vertex attributes.
