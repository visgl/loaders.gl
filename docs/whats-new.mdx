...
**Highlights**
- **Tooling** - CI optimizations
- **TypeScript** Stricter types across code base, migrate to ArrayBufferLike
- **Documentation** - Formatting cleanup, typos etc.

**All modules**

- Worker bundles now exported explicitly in each module's package.json `exports` section.

**@loaders.gl/core**

- Common `LoaderOptions` can now be provided in the `options.core.<...>` rather than on the top level. This allows common options to be type checked. 

**@loaders.gl/mvt**

- `MVTWriter` - A new experimental writer that allows GeoJSON to be written in binary MVT tile format.
