...
- Adds a cross-format [CRS support guide](/docs/developer-guide/coordinate-reference-systems) that
  ...
  roadmap.
- Adds an experimental `SpatialReference` discovery descriptor in `@loaders.gl/loader-utils`,
  adopted by I3S/3D Tiles and FlatGeobuf source metadata without removing their compatibility
  fields or implying that coordinates were transformed.
