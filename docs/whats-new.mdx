...
- Experimental [`WMSService`](modules/wms/api-reference/wms-service) class improvements
  ...
  - Transparency support: The `transparent` prop is now included in the exported WMS parameter types enabling requesting images that are transparent where there is no data.
- `WMSCapabilitiesLoader` handles 1.3.0, and extracts more information (such as `version`, `abstract` fields, layer flags (`opaque`, `queryable`, `cascaded`), MIME types for `exceptions`).
