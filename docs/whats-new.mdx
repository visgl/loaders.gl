...
Feature Highlights:
- **Typed loaders** The **`load()`** and **`parse()`** APIs now infer strict types for loader options and return data from the supplied loader object.
- **Serializable schema types** that support serialization and deserialization (e.g. between workers and main-thread).
- **Writers** - writers are now provided for a few formats, such as CSV, GeoJSON etc.
- **Node.js v18** support
- **Apache Arrow JS v13**
- **[File format documentation](/docs/formats?README)** - new articles provide information about the file formats supported by loaders.gl loaders.

### 4.0 Core Module Updates
...
### 4.0 Loader Module Updates

**@loaders.gl/gltf**

- New glTF 2.0 extensions support:
  - 3DTiles v1.1 attributes supported through [EXT_mesh_features](https://github.com/CesiumGS/glTF/blob/proposal-EXT_mesh_features/extensions/2.0/Vendor/EXT_mesh_features/README.md) and [EXT_mesh_features](https://github.com/CesiumGS/glTF/blob/proposal-EXT_mesh_features/extensions/2.0/Vendor/EXT_mesh_features/README.md)

**@loaders.gl/arrow**

- Upgrades to latest `apache-arrow` v13. 
...
- Upgrades decoders to [draco3d `v1.5.6`](https://github.com/google/draco#version-156-release).
...
- `ZipArchive` - can now open large zip archives and read only index or individual paths without loading entire file (in Node.js).

### 4.0 Tile Module Updates
