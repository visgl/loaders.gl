...
- The root `CSVLoader` and `CSVArrowLoader` exports are now metadata-only loaders that work with core async APIs through dynamic preload.
- `CSVLoader` and `CSVArrowLoader` are available from `@loaders.gl/csv/unbundled` as metadata-only loaders that dynamically import parser-bearing implementations on first async use. See [Using Unbundled Loaders](/docs/developer-guide/using-unbundled-loaders).
- `CSVLoader` and `CSVArrowLoader` are also available from `@loaders.gl/csv/bundled` as explicit parser-bearing imports.
