...
- **Built-in module workers** - Worker-enabled loaders can provide a synchronous `loadWorker()` hook so browser bundlers can load module workers directly from package source, starting with Arrow triangulation, while explicit `workerUrl` overrides and pre-built CDN workers remain supported.
...
- `loadWorker` NEW - worker objects can provide a built-in synchronous worker factory for bundler-resolved module workers. Worker resolution still prioritizes explicit `workerUrl` options and `_workerType: 'test'` before trying `loadWorker`, then falls back to generated pre-built worker URLs.
