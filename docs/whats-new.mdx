...
- Add support for `intensity` and `label` attributes.

**@loaders.gl/ply**

- Add support for custom attributes such as `intensity` and `confidence`.

**@loaders.gl/xml**

- New [`HTMLLoader`](modules/xml/api-reference/html-loader) - a minimal HTML loader, parses HTML as an unstructured XML tree. Intended for minimal ad-hoc use cases such as extracting an error message or a link from a short HTML page.
...
- Experimental [`WMSService`](modules/wms/api-reference/wms-service) class improvements
  - Default WMS parameters (`layers`, `crs` etc) can be specified during `WMSService` construction, instead of having to repeatedly pass them to `getMap` etc methods.
  - WMS 1.3.0 is now the default, and automatic remapping of parameter names is handled under the hood (`CRS` s `SRS`)
  - Authentication support: Plumbs through loadOptions, including HTTP `Authentication` headers.
  - Transparency support: The `transparent` prop is now included in the exported types enabling images that are transparent where there is no data.
- `WMSCapabilitiesLoader` handles 1.3.0, and extracts more information (such as layer flags, exception MIME types).
...
- The [KHR_texture_transform](https://github.com/KhronosGroup/glTF/blob/main/extensions/2.0/Khronos/KHR_texture_transform/README) glTF extension is now supported by the `GLTFLoader`, by optionally transforming UV coordinates on CPU level.
...
- New experimental [`ParquetLoader`](modules/parquet//api-reference/parquet-loader)
