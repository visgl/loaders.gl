...
- Improved typings:`load` and `parse` APIs now infer strict types for both loader options and parsed data from the supplied loader object.
- Integrates with Apache Arrow JS v9+
- `Writers` - new writers for several formats.
- Schema types that support cross-worker schema and table exchange (a "serialized Apache Arrow" representation).
- [File format documentation](/docs/formats?README) - provides additional informationa about the various file formats supported by loaders.gl.
...
- `DeflateCompression` and `GzipCompression` now use [`fflate`](https://101arrowz.github.io/fflate/) instead of [`pako`](http://nodeca.github.io/pako/), providing signficantly better performance and smaller bundle size.

**@loaders.gl/gltf**

- Stricter typing (see upgrade-guide)

**loaders.gl/polyfills**

- Polyfills no longer installs a 
