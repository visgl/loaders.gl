---
title: What's new
description: Follow the latest loaders.gl capabilities, compatibility changes, and format support.
hide_title: true
page_style: designed
---
...
<DocPageHeader
  ...
  title="See what changed, then choose your next path."
  description="The release notes collect new loaders, writers, sources, format support, performance work, and compatibility changes. Start with the current release, then follow the links into the detailed module pages."
  tone="violet"
  meta={['v5.0 in development', 'New loaders and sources', 'Compatibility notes']}
  links={[
...
<DocOrientation
  eyebrow="How to use these notes"
  title="Find the change that affects your application."
  description="Release notes are grouped first by capability and then by module. Use them to discover a feature, then open the linked API or guide for installation, usage, and current support boundaries."
  tone="violet"
  items={[
    {label: 'New capability', value: 'Start with the linked guide or module overview'},
    {label: 'Behavior change', value: 'Read the upgrade guide before changing code'},
    {label: 'Format support', value: 'Check the format page and compatibility matrix'},
    {label: 'Historical detail', value: 'Browse older release sections when tracking a migration'}
  ]}
/>

<ReferenceBoundary
  title="Detailed release history"
  description="The release-by-release notes below preserve implementation detail and links for v5 and earlier versions."
  tone="violet"
/>
...
- [Declarative authentication](/docs/developer-guide/authentication#declarative-credentials)
  resolves JSON credentials through application- or loader-supplied classes. Generic request
  callbacks support per-tile signing and URL rewriting without adding S3/AWS dependencies.

- Fix tile-converter KTX2 decoding and generation under Node.js, and preserve feature IDs
  read from image textures when converting 3D Tiles to I3S.

- Fix binary-to-GeoJSON conversion of multipolygons with empty parts, preserving empty
  polygons without overflowing the call stack.

### 3D Tiles

- The website 3D Tiles example now preserves the tileset center's elevation when initializing
  its camera, while honoring explicit camera positions. See
  [camera elevation guidance](/docs/modules/3d-tiles/concepts/coordinate-reference-systems#camera-elevation-and-high-altitude-content)
  for viewing high-altitude content at close zoom.

- Add [Gaussian primitive descriptors](/docs/modules/3d-tiles/concepts/gaussian-vector-voxel-extensions) with per-primitive validation and preserved accessor/scene-graph references for mixed mesh and splat content.
- Extend standalone [`SPZLoader`](/docs/modules/splats/api-reference/spz-loader) to v2/v3/v4 with explicit RUB/LUF coordinate options and unchanged Arrow-table output.
- Add an optional embedded SPZ2 decoder callback with an explicit LUF source-coordinate hint; compressed bytes stay available when loaded. Attribute conversion and decoded-count reconciliation remain consumer responsibilities.
- Add metadata-first voxel primitive descriptors and 1.x `3DTILES_content_voxels` recognition; dense decoding and voxel rendering remain deferred.
- Improve [SSE/LOD](/docs/modules/3d-tiles/concepts/screen-space-error-and-lod) with transform-scaled 1.x error, orthographic pixel scales, and logical/CSS-pixel selection independent of DPR.
- Add skip-LOD traversal, [progressive/foveated request priority](/docs/modules/3d-tiles/concepts/request-scheduling-and-priorities), and [byte-based cache controls](/docs/modules/3d-tiles/concepts/caching-and-memory).
- Expand [resource handling](/docs/modules/3d-tiles/concepts/resource-resolution-and-content-detection) with structure-first detection, ordered multiple contents, nested packages, and archive-backed resources.
- Add [CRS/epoch discovery and reprojection](/docs/modules/3d-tiles/concepts/coordinate-reference-systems), including nonlinear content/bounds transformation and nested CRS placement; dynamic cross-epoch execution remains excluded.
- Implicit traversal conformance now covers sparse QUADTREE/OCTREE boundaries, multiple content streams, concurrent request deduplication, retry after failure, and final-level availability; lifecycle guarantees are documented.

- Add experimental automatic detection and traversal for the pinned glTF-based 3D Tiles 2.0
  draft, including explicit/implicit hierarchies, spatial metadata, conservative bounds, and a
  shared vector-topology contract with the Cesium 1.1 preview encoding. Draft-facing APIs are
  explicitly experimental; loaders.gl exposes topology and metadata without claiming renderer
  support for drawing, styling, clipping, draping, or clamping.
...
- Decode structural-metadata property-table columns and add normalized 3D Tiles row access with default/noData handling while preserving raw schema, values, and extension references.
...
- Add [renderer-neutral contracts](/docs/modules/3d-tiles/concepts/renderer-contracts-and-metadata) for ordered multi-content entries, feature-ID declarations, raw metadata context, and indexed or union content visibility.
- Add [style-input helpers](/docs/modules/3d-tiles/concepts/styling-and-feature-access) for deterministic metadata precedence, decoded structural-metadata property lookup, and hierarchy-aware batch-table rows without evaluating renderer styling expressions.
- Add deterministic [traversal snapshots](/docs/modules/3d-tiles/concepts/observability-and-benchmarks) with sorted selection/request sets, tile/cache counters, resident-byte estimates, and implicit-subtree diagnostics, plus benchmark-budget guidance.
...
- **Documentation** - Interactive graphical docs explain loaders.gl concepts, loader pages have live previews, related pages are navigable using tabs.
- **Apache Arrow Support** - All loaders and writers now return/accept Apache Arrow tables via `shape: 'arrow-table'`.
- **Unbundled loaders** - Loader modules can expose metadata-only loaders from `/unbundled` subpaths, using imports such as `import {CSVLoader} from '@loaders.gl/csv/unbundled';`, so applications can support rarely used formats without putting every parser implementation in the main bundle.
- **Benchmarks** - Loaders have live benchmarks, some include external parser baselines such as uDSV, d3-dsv, csv-parser, and PapaParse for the CSVLoader allowing comparison of loaders.gl parsing performance against ecosystem SOTA (State-of-the-Art).
- **Coverage expansion and test modernization** - Broad, fast, hermetic Vitest coverage has raised line coverage above 90% and combined line-and-branch coverage by more than 16 percentage points, from about 70% to approximately 86%, while completing the migration of legacy Tape tests to native Vitest syntax and continuing toward the long-term 90% combined-coverage goal.
- **Mesh category writers** - New experimental [`LASWriter`](/docs/modules/las/api-reference/las-writer), [`OBJWriter`](/docs/modules/obj/api-reference/obj-writer), [`PCDWriter`](/docs/modules/pcd/api-reference/pcd-writer), [`PLYWriter`](/docs/modules/ply/api-reference/ply-writer), and [`QuantizedMeshWriter`](/docs/modules/terrain/api-reference/quantized-mesh-writer) add export support for common mesh and point-cloud formats. Mesh writers accept both plain `Mesh` data and Mesh Arrow tables.
- **Raster sources** - New experimental `RasterSource` APIs add viewport-driven typed raster loading, with [`GeoTIFFSourceLoader`](/docs/modules/geotiff/api-reference/geotiff-source-loader), [`OMETiffSourceLoader`](/docs/modules/geotiff/api-reference/ometiff-source-loader), [`OMEZarrSourceLoader`](/docs/modules/zarr/api-reference/ome-zarr-source-loader), [`RasterSet`](/docs/modules/tiles/api-reference/raster-set), and new GeoTIFF / OME-TIFF / OME-Zarr website examples.
...
- **Module workers by default** - Worker descriptors can create bundler-resolved ES module workers as their default browser path while retaining explicit worker URLs and pre-built classic-worker fallbacks. Arrow triangulation is the first API to use this path; existing classic workers are not deprecated.
- **Automatic atomic worker selection** - `core.worker: 'auto'` lets loaders provide synchronous normalized work estimates so small parses stay on the main thread while expensive parses use workers. The default `core.worker: true` behavior remains unchanged.
...
- Adopts the shared `@math.gl/crs` identifier, definition, PROJJSON, WKT, and PROJ types across
  loader metadata and service source APIs.
- `WKTCRSLoader` and `WKTCRSWriter` are thin adapters over the dependency-free math.gl WKT codec
  and use its value-preserving `WKTCRSAst` result.
- GeoParquet metadata distinguishes omitted default CRS, explicit `null`, PROJJSON, and coordinate
  epoch; GeoArrow field metadata types discriminate PROJJSON, WKT2:2019, authority codes, SRIDs,
  and opaque strings.
- Adds a cross-format [CRS support guide](/docs/developer-guide/coordinate-reference-systems) that
  ...
  roadmap.
- Upgrades all math.gl packages to `4.2.0-alpha.6` and adopts the canonical `SpatialReference`
  descriptor from `@math.gl/crs` in I3S/3D Tiles and FlatGeobuf source metadata, without removing
  format compatibility fields or implying that coordinates were transformed.
- Uses math.gl's explicit horizontal-component conversion when a 3D transformation passes a broad
  PROJJSON definition to proj4js, rejecting unsupported CRS object types instead of relying on
  unchecked type casts.
- Aligns tile and deck-layer culling with alpha.6's typed `outside`, `intersecting`, and `inside`
  results.
...
- Vector loaders share `gis: {reproject, targetCrs}` (the experimental `_targetCrs` spelling is no
  longer public), while service feature requests use `requestCrs` for the input bounding box and
  `crs` for returned features. Canonical application coordinates remain `xy`; protocol adapters
  apply wire-axis rules and report opt-in failures as `CRSReprojectionError`.
...
- I3S mesh and Point Cloud sources normalize vertical units and offsets, apply all
  `elevationInfo` placement modes with application terrain or scene providers, and use registered
  `@math.gl/geoid` models for ellipsoidal/orthometric conversion. The [vertical coordinate systems
  guide](/docs/developer-guide/vertical-coordinate-systems) documents formulas, provider
  contracts, and deterministic failure behavior.

**@loaders.gl/arrow**

- `convertArrowVariableWidthVector` and `convertArrowTableVariableWidthTypes` convert `Utf8`,
  `Utf8View`, `Binary`, and `BinaryView` data between standard and view layouts with runtime
  fallback.
...
- [`getStructuralMetadataRow()`](/docs/modules/3d-tiles/concepts/correctness-and-conformance) exposes normalized property-table rows with `default`/`noData` handling while preserving raw metadata references.
...
- [`GLTFLoader`](/docs/modules/gltf/api-reference/gltf-loader) selects `EXT_texture_avif` image sources when the active image decoder supports AVIF, and applies independent `KHR_texture_transform` transforms for textures that share a UV attribute.
...
- TypeScript LAS parses now attach typed VLR/EVLR, Extra Bytes, waveform-descriptor, WKT, and GeoTIFF metadata to `loaderData.metadata`.

**@loaders.gl/obj**

- [`OBJLoader`](/docs/modules/obj/api-reference/obj-loader) adds batched parsing for vertex-only point-cloud OBJ inputs, including `obj.pointCloud: true` for explicit streaming of `v` records and atomic fallback when face or line records are present.
- [`OBJWriter`](/docs/modules/obj/api-reference/obj-writer) NEW - writes Wavefront OBJ mesh data.

**@loaders.gl/pcd**

- [`PCDLoader`](/docs/modules/pcd/api-reference/pcd-loader) adds batched parsing for ASCII and uncompressed binary point clouds, yielding `ArrowTableBatch` values when `pcd.shape: 'arrow-table'` is enabled.
- [`PCDWriter`](/docs/modules/pcd/api-reference/pcd-writer) NEW - writes ASCII Point Cloud Data (PCD).

**@loaders.gl/ply**

- [`PLYWriter`](/docs/modules/ply/api-reference/ply-writer) NEW - writes ASCII Polygon File Format (PLY).
- [`PLYLoader`](/docs/modules/ply/api-reference/ply-loader) supports `ply.shape: 'arrow-table'` for Mesh Arrow table output and preserves Gaussian splat columns such as `f_dc_*`, `opacity`, `scale_*`, and `rot_*` with schema metadata for rendering.
- [`PLYLoader`](/docs/modules/ply/api-reference/ply-loader) now returns loaders.gl `ArrowTableBatch` values from batched Arrow table parsing, enabling progressive point cloud and Gaussian splat rendering.
...
- `CSVLoader` supports `csv.shape: 'arrow-table'` for Apache Arrow table output. It emits Arrow `Utf8` columns by default and uses `csv.dynamicTyping: true` to opt into typed Arrow columns.
- `CSVLoader` adds `csv.viewTypes` to opt into `Utf8View` columns when the installed Apache Arrow runtime supports them, with Arrow 17-compatible fallback behavior.
- `CSVLoader` adds a byte-oriented Arrow parse path for supported raw UTF-8 CSV inputs, including `parse(ArrayBuffer)` and the default unbatched `parseInBatches` path.
- The root `CSVLoader` export is now a metadata-only loader that works with core async APIs through dynamic preload.
- `CSVLoader` is available from `@loaders.gl/csv/unbundled` as a metadata-only loader that dynamically imports the parser-bearing implementation on first async use. See [Using Unbundled Loaders](/docs/developer-guide/using-unbundled-loaders).
- `CSVLoader` is also available from `@loaders.gl/csv/bundled` as an explicit parser-bearing import.
...
- `SplatLayer` adds an experimental WebGPU path with screen-space covariance rendering, frustum and alpha culling, GPU projection/binning, and tile-local sorting for large splat scenes.
- `SplatLayer` also provides a CPU/WebGL fallback path for compatibility, while WebGPU picking remains disabled for the initial implementation.
...
- [`ParquetDatasetSource`](/docs/modules/parquet/api-reference/parquet-source-loader#multi-file-datasets) NEW - coordinates lazy catalog-backed file discovery, conservative file pruning, bounded concurrent Parquet reads, and deterministic Arrow batches. The [Overture GeoParquet example](/examples/geospatial/overture-parquet) runs the complete STAC-to-map query in the browser.
...
- `ParquetSourceLoader` reads column and offset indexes for non-repeated primitive leaves, including struct children, converts page statistics into candidate row ranges, and fetches only the required data-page ranges before exact filtering on the caller thread or worker.
- GeoParquet 1.1 bounding-box coverings now power `ParquetSource.read({bbox})` row-group, nested-page, and exact per-row bbox filtering.
- Native Parquet `GEOMETRY`/`GEOGRAPHY` statistics now power GeoParquet 2.0 row-group pruning. The TypeScript writer emits native logical annotations, XY/XYZ/XYM/XYZM type codes, X/Y/Z/M bounds, and all standardized non-planar edge algorithms.
...
- `ParquetSourceLoader` can transfer selected compressed column chunks to a dedicated worker for decompression, page decoding, column materialization, and Arrow conversion. It returns Arrow tables through `@loaders.gl/arrow`'s direct transferable-buffer transport rather than Arrow IPC.
...
- Zod schemas now cover glTF 1.0, glTF 2.0, draft glTF 2.1, and every official Khronos extension schema fragment. Standalone version schemas, an opt-in all-version union, and individually addressable extension JSON Schemas are published for Monaco and other editor integrations.
...
- Arrow-producing JSON loaders add `arrowConversion.viewTypes` to opt into `BinaryView` and `Utf8View` columns without raising the minimum supported Apache Arrow version.
...
- [`ShapefileLoader`](/docs/modules/shapefile/api-reference/shapefile-loader) supports `shapefile.shape: 'arrow-table'` for Arrow tables with DBF property columns and a WKB `geometry` column annotated with geospatial schema metadata.

**@loaders.gl/flatgeobuf**

- [`FlatGeobufLoader`](/docs/modules/flatgeobuf/api-reference/flatgeobuf-loader) adds `flatgeobuf.shape: 'arrow-table'` with FlatGeobuf property columns, a WKB `geometry` column, and geospatial Arrow metadata for round-tripping to GeoJSON.
- [`FlatGeobufSourceLoader`](/docs/modules/flatgeobuf/api-reference/flatgeobuf-source-loader) NEW - creates indexed remote `.fgb` vector sources with schema/metadata discovery, bbox-filtered HTTP range reads, and `geojson`, `binary`, or `arrow` outputs.

**@loaders.gl/json**

- [`GeoJSONLoader`](/docs/modules/json/api-reference/geojson-loader), `GeoJSONWorkerLoader`, and [`GeoJSONWriter`](/docs/modules/json/api-reference/geojson-writer) are now official exports.
- [`GeoJSONLoader`](/docs/modules/json/api-reference/geojson-loader) now selects output with `geojson.shape`, including `'geojson-table'`, `'binary-feature-collection'`, and `'arrow-table'`.
- [`GeoJSONLoader`](/docs/modules/json/api-reference/geojson-loader) adds GeoArrow WKB Arrow output through `geojson.shape: 'arrow-table'`; GeoJSON remains a geospatial format with dedicated loader semantics even though it uses JSON encoding.
- [`JSONTableLoader`](/docs/modules/json/api-reference/json-table-loader) is the dedicated table-returning JSON API. [`JSONLoader`](/docs/modules/json/api-reference/json-loader) remains the arbitrary JSON document loader.
...
- **@loaders.gl/mlt** - New [`MLTLoader`](/docs/modules/mlt/api-reference/mlt-loader) and [`MLTSourceLoader`](/docs/modules/mlt/api-reference/mlt-source-loader) add support for the [`MapLibre Tile`](/docs/modules/mlt/formats/mlt) format, with a new [MLT website example](/examples/tiles/mlt).
- **@loaders.gl/textures** - Texture loaders now return `TextureLevel` objects with explicit `shape`, `format`, and `textureFormat` fields, add manifest-driven [`TextureLoader`](/docs/modules/textures/api-reference/texture-loader), [`TextureArrayLoader`](/docs/modules/textures/api-reference/texture-array-loader), and [`TextureCubeLoader`](/docs/modules/textures/api-reference/texture-cube-loader), and support Radiance HDR textures through [`RadianceHDRLoader`](/docs/modules/textures/api-reference/radiance-hdr-loader).
- **@loaders.gl/arrow** - Added support for `apache-arrow@21` and a new guide for [Using with Apache Arrow](/docs/developer-guide/apache-arrow).
- **@loaders.gl/mvt** - New experimental [`MVTWriter`](/docs/modules/mvt/api-reference/mvt-writer) allows GeoJSON to be written in binary MVT tile format.
...
- [`TableTileSourceLoader`](/docs/modules/mvt/api-reference/table-tile-source-loader) - Geospatial tables can now be vector tiled dynamically in the browser.
...
- **Native 3D Tiles test suites** - Active 3D Tiles parser and runtime suites now use native Vitest assertions, recover composite-i3dm and SSE coverage, and remove thousands of lines of non-executable Cesium renderer specifications from active test files.
...
- **Lazy implicit initialization** Implicit tilesets complete root parsing and `onTilesetLoad` without fetching subtree availability resources or eagerly allocating the complete hierarchy.
- **View-gated subtree requests** Implicit availability is requested only after frustum, viewer-request-volume, and SSE checks, using the same progressive, foveated, and concurrency priorities as tile content.
- **Bounded subtree materialization** Each request materializes exactly one QUADTREE or OCTREE subtree, leaves deeper boundaries lazy, deduplicates final URLs, and retains a configurable parsed-subtree LRU.
- **Correct implicit level semantics** `availableLevels` is treated as a count including level zero, child-subtree Morton indexes omit the tile-level offset, and the last declared level is materialized without requesting beyond it.
- **Implicit hierarchy diagnostics** Runtime child states and `Tiles3DSource.getImplicitTilingStats()` expose pending requests, cache reuse, and materialized-header counts, documented in a new [Implicit tab](/docs/modules/3d-tiles/concepts/implicit-tiling-and-subtrees).
...
- `PMTilesSourceLoader` - now uses [`pmtiles` version 3](https://github.com/protomaps/PMTiles/blob/main/js/CHANGELOG.md).
- `PMTilesSourceLoader` - Now refreshes the already loaded index if the underlying PMTiles file being loaded from is changed.
...
- New [`PMTilesSourceLoader`](/docs/modules/pmtiles/api-reference/pmtiles-source-loader) supports the [PMTiles](/docs/modules/pmtiles/formats/pmtiles) format.
...
- [`WMSService`](modules/wms/api-reference/wms-source-loader)
...
- [`CSWService`](modules/wms/api-reference/csw-source-loader)
...
>
  <figure style={{margin: 0, maxWidth: 240, textAlign: 'center'}}>
    <img
      style={{maxHeight: 200, maxWidth: '100%'}}
      src="https://raw.github.com/visgl/deck.gl-data/master/images/whats-new/mvt-layer.jpg"
      alt="Mapbox Vector Tile layer"
    />
    <figcaption>
      <em>{'MVTLoader + '}<a target="_blank" rel="noreferrer" href="https://deck.gl/#/documentation/deckgl-api-reference/layers/mvt-layer">{'MVTLayer'}</a>{' (v2.1)'}</em>
    </figcaption>
  </figure>
  <figure style={{margin: 0, maxWidth: 240, textAlign: 'center'}}>
    <img
    ...
    />
    <figcaption>
      <em>{'TerrainLoader + '}<a target="_blank" rel="noreferrer" href="https://deck.gl/#/documentation/deckgl-api-reference/layers/terrain-layer">{'TerrainLayer'}</a>{' (v2.1)'}</em>
    </figcaption>
  </figure>
  <figure style={{margin: 0, maxWidth: 240, textAlign: 'center'}}>
    <img
    ...
    />
    <figcaption>
      <em>{'I3SLoader + '}<a target="_blank" rel="noreferrer" href="https://deck.gl/#/documentation/deckgl-api-reference/layers/tile-3d-layer">{'Tile3DLayer'}</a>{' + I3S (v2.1)'}</em>
    </figcaption>
  </figure>
</div>
...
## ArcGIS data access (v5 development)

The new [ArcGIS module](/docs/modules/arcgis) owns FeatureServer, MapServer, ImageServer,
VectorTileServer and SceneServer adapters, with lazy source loading, credential and discovery
entry points, a [developer guide](/docs/developer-guide/arcgis),
[operation-level support inventory](/docs/modules/arcgis/services), and
[examples gallery](/examples/arcgis). Service pages embed their examples and distinguish data
access from complete ArcGIS rendering fidelity. See the [upgrade guide](/docs/upgrade-guide)
for moved imports.
