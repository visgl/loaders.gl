...
Release Date: TBD, Q3-2022

A major release that brings: 

Feature Highlights:
- Typed loaders: loaders now encode the type of both parsed data and options.
- Type inference: `load*()` and `parse*()` APIs now infer strict types for both loader options and parsed data from the supplied loader object.
- Schema types that support cross-worker schema and table exchange (a "serialized Apache Arrow" representation).
- `Writers` - writers are now provided for a few formats.

Integration:
- Node.js v18 support
- Integration with Apache Arrow JS v9+

Documentation:
- [File format documentation](/docs/formats?README) - a growing list of articles that provide additional information about the various file formats supported by loaders.gl.
...
- Now uses `apache-arrow` v13. 
- Note that Apache Arrow JS v9 was a breaking change to enable tree-shaking, so if your application is using the Apache Arrow JS API directly you may need to do upgrades. 
- Apache Arrow JS v10 through v13 had very few changes.

**@loaders.gl/compression**

- `DeflateCompression` and `GzipCompression` now use [`fflate`](https://101arrowz.github.io/fflate/) instead of [`pako`](http://nodeca.github.io/pako/), providing ~20% better performance and smaller bundle size.
...
- Polyfills no longer installs `fetch`/`Response`/`Headers` polyfills, instead leveraging built-in support in Node.js v18+.
