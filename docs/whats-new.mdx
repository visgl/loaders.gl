...
- Support for `apache-arrow@21`
- A new documentation page about [Using with Apache Arrow](/docs/developer-guide/apache-arrow).
...
- **Maplibre Tile Format Support**
  - [`MLTLoader`](/docs/modules/mlt/api-reference/mlt-loader)
  - [`MLTSource`](/docs/modules/mlt/api-reference/mlt-source)
  - [`MapLibre Tile`](/docs/modules/mlt/formats/mlt) format documentation 
  - [MLT website example](/examples/tiles/mlt).

**@loaders.gl/mvt**

- [`MVTWriter`](/docs/modules/mvt/api-reference/mvt-writer) A new experimental writer that allows GeoJSON to be written in binary MVT tile format.
...
- Texture loaders now return `TextureLevel` objects with explicit `shape`, `format`, and `textureFormat` fields.
- `BasisLoader` now accepts `basis.supportedTextureFormats` to guide `'auto'` format selection with an explicit list of supported target formats.
- [`RadianceHDRLoader`](/docs/modules/textures/api-reference/radiance-hdr-loader) adds support for Radiance RGBE `.hdr` textures and returns a `Texture` with a decoded `rgba32float` level.

**@loaders.gl/zip**

- `ZipWriter` - Can now create directory entries to match file names.

**@loaders.gl/tile-converter**

- `tile-converter` is now broken out into a separate "application" and will be be versioned and published independently.
