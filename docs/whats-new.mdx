...
- **TypeScript**: Loader objects are now typed using `as const satisfies`, which preserves more type information. 
...
- **Build tooling**: loaders.gl builds ~10x faster via esbuild, and installs ~2x faster via yarn 4.
