...
- New: Resume conversion from the last tile processed in case of an interruption.
- New: Quiet mode.
- New: Convert I3S to 3D Tiles from local SLPK file.

**@loaders.gl/pmtiles**

- Now uses [`pmtiles` version 3](https://github.com/protomaps/PMTiles/blob/main/js/CHANGELOG.md).
- New: Now refreshes the already loaded index if a PMTiles file being loaded from is changed (rewritten). 
...
- Fixes: Many parquet files that previously failed to load now load correctly.
...
- `DracoLoader` now supports FLOAT64, INT64, UINT64 types for extra attributes.

**@loaders.gl/3d-tiles**

- Now supports FLOAT64, INT64, UINT64 point cloud attributes imported from LAS. See [CesiumJS blog]( https://cesium.com/blog/2024/03/20/preserving-more-metadata-for-point-clouds-using-3dtiles/)
