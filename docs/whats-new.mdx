...
- [roadmap](./roadmap) for more details on planned development.

## v4.1 (In Early Development)

Release Date: Target Q1 204
...
Release Date: TBD, Target Sep 2023
...
- Since `@loaders.gl/polyfills` no longer modifies the global fetch function to support local filesystem access.

**@loaders.gl/schema**

- The loaders.gl schema and type system is now a fully serializable counterpart to the Apache Arrow schema and type system. 
- Among other things, this simplifies transferring schemas between workers.

**loaders.gl/polyfills**

- Polyfills no longer installs `fetch`/`Response`/`Headers` polyfills under Node.js, instead leveraging built-in support in Node.js v18+.
...
- Be aware that Apache Arrow JS v9 is a major breaking change compared with Apache Arrow v4 which is used by loaders.gl v3, so if you are using the Arrow API directly you will need to upgrade.

**@loaders.gl/csv**

- New [`CSVWriter`](docs/modules/csv/api-reference/csv-writer) allows export of any table in CSV format.

**@laoders.gl/json**

- New [`JSONWriter`](docs/modules/json/api-reference/json-writer) allows export of any table in JSON format.
- New [`NDJSONWriter`](docs/modules/json/api-reference/ndjson-writer) allows export of any table in NDJSON format.
- New [`GeoJSONWriter`](docs/modules/json/api-reference/geojson-writer) allows export of any table in GeoJSON format.
- New [`NDGeoJSONWriter`](docs/modules/json/api-reference/ndgeojson-writer) allows export of any table in NDGeoJSON format.
