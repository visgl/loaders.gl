...
- Improved typings:`load` and `parse` APIs now infer the strict types from the supplied loader object. Both the returned data and the loader options types will be inferred. 
...
- New `Writers` for a variselection of formats
- Support for cross-worker schema and table exchange, through a "serialized Apache Arrow" representation.
- New documentation section covering file formats, providing more in-depth informationa about the various file formats supported by loaders.gl.
