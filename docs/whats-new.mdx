...
- Add reader-side Gaussian splat extension descriptors, SPZ v2/v3/v4 decoding with explicit coordinate options, and lazy voxel contracts for 3D Tiles; embedded SPZ2 bytes remain available for injected decoders while styling and rendering stay application-owned.
...
- Add normalized 3D Tiles metadata row access with default/noData handling while preserving raw schema and extension references.
...
- Add renderer-neutral style-input helpers for deterministic metadata precedence, decoded structural-metadata property lookup, and hierarchy-aware batch-table rows without evaluating renderer styling expressions.
