...
Release Date: TBD, Target Oct 2023

A major release that brings: 

- **Typed loaders** The **`load()`** and **`parse()`** APIs now infer types from the supplied loader.
- **More Writers** loaders are complemented by writers for formats such as CSV, GeoJSON etc.
- **Node.js v18** support.
- **[File format documentation](/docs/formats?README)** - articles describing the file formats supported by loaders.gl.

### Loader Module Updates

**@loaders.gl/pmtiles** - NEW

- Supports the [PMTiles](/docs/modules/pmtiles/formats/pmtiles) format.
- New experimental [`PMTilesLoader`](/docs/modules/parquet/api-reference/parquet-loader)
- New experimental [`PMTilesService`](/docs/modules/parquet/api-reference/parquet-loader)
- New [PMTiles example](/examples/pmtiles).

**@loaders.gl/parquet** 

- Supports the [Parquet](/docs/modules/parquet/formats/parquet) and [GeoParquet](/docs/modules/parquet/formats/geoparquet) file formats.
- [`ParquetLoader`](/docs/modules/parquet/api-reference/parquet-loader) now supports loading GeoParquet files.
- New [GeoParquet example](/examples/geospatial).
...
- `ZipArchive` - now opens large zip archives without loading entire file (in Node.js).

**@loaders.gl/gltf**

- Support for [EXT_mesh_features](https://github.com/CesiumGS/glTF/blob/proposal-EXT_mesh_features/extensions/2.0/Vendor/EXT_mesh_features/README.md) and [EXT_mesh_features](https://github.com/CesiumGS/glTF/blob/proposal-EXT_mesh_features/extensions/2.0/Vendor/EXT_mesh_features/README.md) gltF extensions for 3D Tiles v1.1.

**@loaders.gl/3d-tiles**

- 3D Tiles v1.1 [Implicit Tiling](https://github.com/CesiumGS/3d-tiles/tree/main/specification/ImplicitTiling)
- 3D Tiles v1.1 attributes (via the new glTF extensions).
...
- Better [I3S archive format (SLPK)](https://github.com/Esri/i3s-spec/blob/master/format/Indexed%203d%20Scene%20Layer%20Format%20Specification.md#scene-layer-packages-) support:
...
- [`WMSCapabilitiesLoader`](modules/wms/docs/api-reference/wms-capabilities-loader)
